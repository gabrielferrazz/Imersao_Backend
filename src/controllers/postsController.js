...
export async function listarPosts(req, res) {
    try {
        ...
        const baseUrl = process.env.BASE_URL; // Pega a BASE_URL do .env
        const updatedPosts = posts.map(post => {
            if (post.imagemUrl.includes("localhost")) {
                return {
                    ...post,
                    imagemUrl: `${baseUrl}/${post._id}.png` // Substitui localhost pela BASE_URL
                };
            }
            return post; // Retorna o post original se a URL já estiver correta
        });
